...
struct ClassInstance {
    ...
    // Guards concurrent field read/write when an instance is shared
    // across `spawn` threads (e.g. captured by two closures).
...
struct Value {
    ...
    // Guards concurrent read/write of `list`/`map` when this Value is
    // shared across `spawn` threads. A plain std::vector/unordered_map
    // is not safe for concurrent mutation (a push_back triggering a
    // reallocation while another thread reads/writes is memory
    // corruption, not just a logical race) — every access site that can
    // run under `spawn` takes this lock. Recursive because a locked
    // operation can call back into code that locks the same Value again
    // (e.g. compound-assignment reading then writing the same index).
    mutable std::recursive_mutex mtx;
    ...
    static ValuePtr make_int(long long i) {
        // Small-int cache: integer literals and loop counters are
        // overwhelmingly small in practice, and Value objects are never
        // mutated in place after construction (assignment/move always
        // replaces the shared_ptr in the Env, never writes through it) —
        // so sharing one Value per small integer is safe and avoids a
        // heap allocation on every arithmetic op / loop iteration.
        constexpr long long kCacheLo = -256, kCacheHi = 1024;
        if (i >= kCacheLo && i < kCacheHi) {
            static std::vector<ValuePtr> cache = [] {
                std::vector<ValuePtr> c;
                c.reserve(kCacheHi - kCacheLo);
                for (long long v = kCacheLo; v < kCacheHi; ++v) {
                    auto val = std::make_shared<Value>();
                    val->kind = ValueKind::Int; val->ival = v;
                    c.push_back(val);
                }
                return c;
            }();
            return cache[(size_t)(i - kCacheLo)];
        }
...
// ─────────────────────────────────────────────
//  Catchable throw — `throw expr;`
//
//  Deliberately NOT a RuntimeError/panic: RuntimeError represents a
//  programmer bug (overflow, out-of-bounds, use-after-move — things
//  Aegis treats as fatal by design) and is never caught by user code.
//  AegisThrow carries an arbitrary Aegis value and unwinds only as far
//  as the nearest enclosing `try`/`catch` (exec_try below); if nothing
//  catches it, it reaches main() and is reported like any other
//  uncaught error.
// ─────────────────────────────────────────────
struct AegisThrow {
    ValuePtr value;
...
private:
    ...
    ValuePtr exec_try     (ASTNode* node, std::shared_ptr<Env> env);

...
private:
    ...
    bool                  in_loop_ = false;
    int                   scope_depth_ = 0;
    // scope_depth_ recorded at the moment we start visiting each currently-
    // active loop's body — lets MoveExpr tell "declared outside this loop
    // (persists across iterations, unsafe to re-move)" from "declared
    // inside the loop body (fresh each real iteration, safe)".
    std::vector<int>      loop_scope_depths_;

    // ── Scope management ──────────────────────
    void   push_scope();
    void   pop_scope();
    Symbol* lookup(const std::string& name);
    int     lookup_scope_depth(const std::string& name);
    void   define(Symbol sym);
    ...
    // True if `derived` is `base` or inherits from it (directly or transitively).
    bool is_subclass_of(const std::string& derived, const std::string& base);
    ...
    void     visit_try   (ASTNode* node);
    void     visit_throw (ASTNode* node);

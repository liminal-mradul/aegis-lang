...
// ─────────────────────────────────────────────
//  Module resolution — `use ./mod;`  /  `use ./mod::{a, b};`
//
//  Runs once, right after parsing and before sema/interpret/codegen, so
//  every downstream pass just sees one flat, merged AST and needs no
//  changes of its own. `aegis::io`-style built-in namespaces (bval ==
//  false, handled natively elsewhere) are left completely untouched.
//
//  A relative `use` node is replaced in-place by the top-level
//  declarations of the file it names (filtered to the requested names
//  for `use ./mod::{a, b};`). Files are lexed/parsed/resolved exactly
//  once even if imported from multiple places (tracked via `visited`),
//  and a depth counter catches import cycles.
// ─────────────────────────────────────────────
static bool resolve_modules(ASTNode* root, const std::string& file_path,
                             std::unordered_set<std::string>& visited,
                             int depth) {
    if (!root) return true;
    if (depth > 32) {
        std::cerr << C_ERR->red
                  << "[Aegis] Import depth exceeded 32 — likely a cycle "
                     "in 'use' statements (importing from " << file_path << ")"
                  << C_ERR->reset << "\n";
        return false;
    }
    std::string dir = platform::path_dir(file_path);

    ASTList new_children;
    for (auto& child : root->children) {
        bool is_use = child && (child->kind == NodeKind::UseDecl ||
                                 child->kind == NodeKind::UseFromDecl);
        if (!is_use || !child->bval) {
            new_children.push_back(std::move(child));
            continue;
        }

        size_t path_len = (size_t)child->ival;
        std::vector<std::string> path_segs(
            child->str_list.begin(),
            child->str_list.begin() + std::min(path_len, child->str_list.size()));
        std::vector<std::string> import_names;
        if (child->kind == NodeKind::UseFromDecl && path_len < child->str_list.size())
            import_names.assign(child->str_list.begin() + path_len, child->str_list.end());
        ...
        std::string mod_path = platform::normalise_path(
            platform::path_join(dir, rel + ".ae"));

        if (visited.count(mod_path)) continue;
        visited.insert(mod_path);

        std::ifstream f(mod_path, std::ios::binary);
        if (!f.is_open()) {
            std::cerr << C_ERR->red << "[Aegis] Cannot resolve import '"
                      << rel << "' -> " << mod_path
                      << " (from " << file_path << ")" << C_ERR->reset << "\n";
            return false;
        }
        std::ostringstream ss;
        ss << f.rdbuf();
        std::string mod_source = platform::strip_cr(ss.str());

        Lexer mlexer(mod_source, mod_path);
        auto mtokens = mlexer.tokenize();
        if (mlexer.has_errors()) {
            std::cerr << C_ERR->red << "[Lex Errors] in " << mod_path
                      << C_ERR->reset << "\n";
            for (auto& e : mlexer.errors())
                std::cerr << "  " << C_ERR->red << e.to_string() << C_ERR->reset << "\n";
            return false;
        }
        Parser mparser(std::move(mtokens));
        auto mod_ast = mparser.parse();
        if (mparser.has_errors()) {
            std::cerr << C_ERR->red << "[Parse Errors] in " << mod_path
                      << C_ERR->reset << "\n";
            for (auto& e : mparser.errors())
                std::cerr << "  " << C_ERR->red << e.to_string() << C_ERR->reset << "\n";
            return false;
        }

        if (!resolve_modules(mod_ast.get(), mod_path, visited, depth + 1))
            return false;

        for (auto& mchild : mod_ast->children) {
            if (!mchild) continue;
            std::string decl_name;
            if (mchild->kind == NodeKind::FuncDecl ||
                mchild->kind == NodeKind::AsyncFuncDecl ||
                mchild->kind == NodeKind::ClassDecl ||
                mchild->kind == NodeKind::VarDecl)
                decl_name = mchild->sval;

            if (!import_names.empty() && !decl_name.empty() &&
                std::find(import_names.begin(), import_names.end(), decl_name)
                    == import_names.end())
                continue;

            new_children.push_back(std::move(mchild));
        }
    }
...
static ASTNodePtr compile_frontend(const std::string& source,
    ...
    std::unordered_set<std::string> visited{platform::normalise_path(filename)};
    if (!resolve_modules(ast.get(), filename, visited, 0))
        return nullptr;
    ...
    while (true) {
        ...
        try {
            ...
            std::cerr << C_ERR->red << "[RuntimeError] " << e.message
                      << C_ERR->reset << "\n";
        } catch (const AegisThrow& t) {
            std::cerr << C_ERR->red << "[Uncaught throw] "
                      << (t.value ? t.value->to_display() : "null")
                      << C_ERR->reset << "\n";
        }
    }
}
...
static int cmd_run(const std::string& file) {
    ...
    try {
        ...
        std::cerr << "\n" << C_ERR->red << "[Uncaught throw] "
                  << (t.value ? t.value->to_display() : "null");
        if (t.line > 0) std::cerr << " at line " << t.line << ":" << t.col;
        std::cerr << C_ERR->reset << "\n";
        return 1;
    }
}
...
static int cmd_build(const std::string& raw_file, std::string out_path) {
    ...
    // A native build needs a `main :: () { ... }` entry point — without
    // one, gen_program() emits `global main` but never defines it, which
    // used to fail only much later at the link step with a confusing
    // "undefined reference to `main'" error. Catch it here instead, with
    // a message that actually points at the fix.
    bool has_main = false;
    for (auto& child : ast->children) {
        if (child && (child->kind == NodeKind::FuncDecl ||
                       child->kind == NodeKind::AsyncFuncDecl) &&
            child->sval == "main") {
            has_main = true;
            break;
        }
    ...
    if (!has_main) {
        std::cerr << C_ERR->red
                  << "[Aegis] No entry point found: 'aegis build' needs a "
                     "top-level 'main :: () { ... }' function.\n"
                     "  Interpreted mode ('aegis run') doesn't require this "
                     "— top-level statements run directly — but native "
                     "builds need a named entry point, just like C/C++/Rust."
                  << C_ERR->reset << "\n";
        return 1;
    }

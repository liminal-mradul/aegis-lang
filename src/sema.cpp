...
// The scope depth `name` was actually declared at, or -1 if not found.
// Used to tell "declared inside the current loop body" (safe to move —
// fresh each real iteration) from "declared outside it" (persists across
// iterations — moving it in the loop body is a bug on iteration 2+).
int Sema::lookup_scope_depth(const std::string& name) {
    for (int i = (int)scopes_.size()-1; i >= 0; --i) {
        if (scopes_[i].has(name)) return scopes_[i].depth;
    }
    return -1;
}
...
// Walk the inheritance chain recorded in classes_ (built during visit_class)
// to see whether `derived` IS-A `base`, directly or through any number of
// intermediate parents. Guards against malformed/cyclic chains.
bool Sema::is_subclass_of(const std::string& derived, const std::string& base) {
...
bool Sema::types_compatible(const TypePtr& a, const TypePtr& b) {
    ...
    // Class subtyping: a value of a derived class is assignable wherever
    // its base class (or any ancestor) is expected — e.g. passing a
    // `Circle` where a `Shape` parameter is declared. `a` is the expected/
    // declared type, `b` is the actual/inferred type, so we check that
    // b IS-A a (not the other way around).
    if (a->kind == TypeKind::Class && b->kind == TypeKind::Class) {
        if (is_subclass_of(b->name, a->name)) return true;
    }
...
void Sema::visit_stmt(ASTNode* node) {
    ...
    switch (node->kind) {
        ...
        case NodeKind::TryStmt:     visit_try(node);      break;
        case NodeKind::ThrowStmt:   visit_throw(node);    break;
...
void Sema::visit_while(ASTNode* node) {
    ...
    loop_scope_depths_.push_back(scope_depth_);
    if (node->right) visit_stmt(node->right.get());
    loop_scope_depths_.pop_back();
    in_loop_ = prev;
}
...
void Sema::visit_for(ASTNode* node) {
    ...
    bool prev = in_loop_; in_loop_ = true;
    loop_scope_depths_.push_back(scope_depth_ - 1); // include the loop var's own scope as "inside"
    if (node->right) visit_stmt(node->right.get());
    loop_scope_depths_.pop_back();
    in_loop_ = prev;
    pop_scope();
...
//  try { ... } catch [name] { ... }
//
//  The try block and catch block are each their own scope. The catch
//  variable (if named) is typed Unknown — a thrown value can be
//  anything, so we don't constrain it further; the catch block treats
//  it like any other dynamically-typed binding.
// ─────────────────────────────────────────────
void Sema::visit_try(ASTNode* node) {
    push_scope();
    if (node->left) visit_stmt(node->left.get());
    pop_scope();
    ...
    push_scope();
    if (!node->sval.empty()) {
        Symbol s(node->sval, SymbolKind::Variable,
                  Type::make_named(TypeKind::Unknown, ""), false, node->loc);
        s.is_init = true;
        define(s);
    }
    if (node->extra) visit_stmt(node->extra.get());
    pop_scope();
}
...
//  throw expr;
// ─────────────────────────────────────────────
void Sema::visit_throw(ASTNode* node) {
    if (node->left) infer(node->left.get());
}
        ...
        case NodeKind::MoveExpr: {
            ...
            if (node->left && node->left->kind == NodeKind::Ident) {
                ...
                if (sym) {
                    ...
                    // Bug fix: moving an already-moved value is itself a
                    // use-after-move — previously this fell through
                    // silently and just re-set is_moved (already true).
                    else if (sym->is_moved)
                        error("Use of moved value '" + node->left->sval +
                              "' — cannot move it again, ownership has "
                              "already been transferred", node->loc);
                    else
                        sym->is_moved = true;

                    // Loop re-move check: if this variable was declared
                    // outside the innermost active loop, it persists
                    // across real iterations — moving it here means a
                    // second iteration would move an already-moved value.
                    // (Heuristic, not a hard error: `break` after the
                    // move, or a reassignment before it, can make this
                    // safe — hence a warning, like the non-exhaustive
                    // `match` check elsewhere in this file.)
                    if (!loop_scope_depths_.empty()) {
                        int sym_depth = lookup_scope_depth(node->left->sval);
                        if (sym_depth >= 0 && sym_depth < loop_scope_depths_.back()) {
                            warning("'" + node->left->sval + "' is moved inside a loop "
                                    "but declared outside it — if this loop runs more "
                                    "than once, a later iteration will try to move an "
                                    "already-moved value", node->loc);
                        }
                    }
                }
            }
    ...
    if (node->left) {
        ...
        if (!target_name.empty()) {
            ...
            if (sym) {
                ...
                // A plain `=` gives the variable a brand-new value, so
                // any earlier move() no longer applies — without this,
                // `move(a, b); a = own<int>(10); print(a);` was wrongly
                // rejected as a use of the *old*, already-moved value.
                if (!is_compound) sym->is_moved = false;

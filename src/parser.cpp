...
ASTNodePtr Parser::parse_use_decl() {
    ...
    // Relative import — `./mod` — resolved to an on-disk .ae file by the
    // module loader (main.cpp) before sema/interpret/codegen ever run.
    // `aegis::io`-style paths (bval == false) refer to built-in namespaces
    // handled natively by the interpreter/codegen and are left untouched.
    node->bval = (check(TokenType::DOT) && check2(TokenType::SLASH));
    // collect path segments
    node->str_list = parse_use_path();
    // Path segment count, so a later selective-import block (below) can
    // append import names onto the same str_list without ambiguity:
    // str_list[0 .. ival) is the module path, [ival .. end) are names.
    node->ival = (long long)node->str_list.size();
...
ASTNodePtr Parser::parse_stmt() {
    ...
    if (t == TokenType::TRY)    return parse_try_stmt();
    if (t == TokenType::THROW)  return parse_throw_stmt();
...
//  try { ... } catch [name] { ... }
//
//  `catch e { ... }` binds the thrown value to `e` inside the catch
//  block; the bare `catch { ... }` form (no binding) is also accepted
//  for callers who don't care about the thrown value.
// ─────────────────────────────────────────────
ASTNodePtr Parser::parse_try_stmt() {
    auto loc = here();
    expect(TokenType::TRY, "'try'");
    auto node  = make_node(NodeKind::TryStmt, loc);
    node->left = parse_block();
    expect(TokenType::CATCH, "'catch'");
    if (check(TokenType::IDENT)) {
        node->sval = advance().value;
    }
...
//  throw expr;
// ─────────────────────────────────────────────
ASTNodePtr Parser::parse_throw_stmt() {
    auto loc = here();
    expect(TokenType::THROW, "'throw'");
    auto node  = make_node(NodeKind::ThrowStmt, loc);
    node->left = parse_expr();
    match(TokenType::SEMICOLON);
    return node;
}

...
void Interpreter::register_builtins(std::shared_ptr<Env> env) {
    ...
    env->define("len", Value::make_native([](std::vector<ValuePtr> args) -> ValuePtr {
        ...
        auto& a = args[0];
        std::lock_guard<std::recursive_mutex> lk(a->mtx);
    ...
    env->define("push", Value::make_native([](std::vector<ValuePtr> args) -> ValuePtr {
        ...
        std::lock_guard<std::recursive_mutex> lk(args[0]->mtx);
...
ValuePtr Interpreter::exec_stmt(ASTNode* node, std::shared_ptr<Env> env) {
    ...
    switch (node->kind) {
        ...
        case NodeKind::TryStmt:      return exec_try(node, env);
        case NodeKind::ThrowStmt: {
            ValuePtr val = node->left ? eval(node->left.get(), env) : Value::make_null();
            throw AegisThrow{val, node->loc.line, node->loc.col};
        }
...
ValuePtr Interpreter::exec_for(ASTNode* node, std::shared_ptr<Env> env) {
    // Fast path: `for i in a..b { }` / `for i in a..=b { }` — iterate the
    // bounds directly instead of building a full materialized list first.
    // For large ranges this avoids allocating (and then copying) a
    // multi-million-element std::vector<ValuePtr> just to walk it once.
    if (node->left && node->left->kind == NodeKind::RangeExpr) {
        ASTNode* range = node->left.get();
        ValuePtr lo = range->left  ? eval(range->left.get(),  env) : nullptr;
        ValuePtr hi = range->right ? eval(range->right.get(), env) : nullptr;
        long long start = lo ? lo->ival : 0;
        long long end   = hi ? hi->ival : 0;
        if (range->bval) ++end; // inclusive ..= range

        for (long long i = start; i < end; ++i) {
            auto loop_env = std::make_shared<Env>(env);
            loop_env->define(node->sval, Value::make_int(i), /*immutable=*/true);
...
ValuePtr Interpreter::exec_try(ASTNode* node, std::shared_ptr<Env> env) {
    // Own scope for the try block, so anything it defines doesn't leak
    // into the catch block (mirrors how if/while/for bodies scope).
    try {
        auto try_env = std::make_shared<Env>(env);
        return exec_block(node->left.get(), try_env);
    } catch (AegisThrow& t) {
        auto catch_env = std::make_shared<Env>(env);
        if (!node->sval.empty())
            catch_env->define(node->sval, t.value ? t.value : Value::make_null(), false);
        return exec_block(node->extra.get(), catch_env);
    }
    // Note: RuntimeError (panics — overflow, out-of-bounds, use-after-move,
    // stack overflow, ...) is deliberately NOT caught here. Those represent
    // programmer bugs Aegis treats as fatal by design; only values raised
    // with an explicit `throw` are recoverable.
}
...
ValuePtr Interpreter::eval_assign(ASTNode* node, std::shared_ptr<Env> env) {
    ...
    if (node->left->kind == NodeKind::FieldExpr) {
        ...
        if (obj && obj->kind == ValueKind::Class && obj->instance) {
            ...
            std::lock_guard<std::recursive_mutex> lk(obj->instance->mtx);
    ...
    if (node->left->kind == NodeKind::IndexExpr) {
        ...
        if (arr && idx) {
            if (arr->kind == ValueKind::List) {
                std::lock_guard<std::recursive_mutex> lk(arr->mtx);
    ...
    if (node->left && node->left->kind == NodeKind::FieldExpr) {
        ...
        if (obj && obj->kind == ValueKind::Class && obj->instance) {
            ValuePtr fn;
            {
                ...
                if (it != obj->instance->fields.end()) fn = it->second;
            }
            if (fn) return call_function(fn, args, env, obj);
        }
        if (obj && (obj->kind == ValueKind::Map || obj->kind == ValueKind::Thread)) {
            ValuePtr fn;
            {
                std::lock_guard<std::recursive_mutex> lk(obj->mtx);
                auto it = obj->map.find(method);
                if (it != obj->map.end()) fn = it->second;
            }
            if (fn) return call_function(fn, args, env);
        }
...
ValuePtr Interpreter::eval_field(ASTNode* node, std::shared_ptr<Env> env) {
    ...
    if (obj->kind == ValueKind::List  && field == "len") {
        std::lock_guard<std::recursive_mutex> lk(obj->mtx);
        return Value::make_int((long long)obj->list.size());
    }

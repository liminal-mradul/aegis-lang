...
void Codegen::gen_stmt(ASTNode* node) {
    ...
    switch (node->kind) {
        ...
        case NodeKind::ThrowStmt: {
            // try/catch/throw is interpreter-only for now (see README's
            // support matrix) — warn once instead of silently dropping
            // the block, so a `build` doesn't quietly produce a binary
            // that skips error-handling code the source clearly has.
            static std::once_flag try_warned;
            std::call_once(try_warned, []() {
                std::cerr << "[Warning] 'try'/'catch'/'throw' are interpreter-only "
                             "and are not compiled to native code — "
                             "run this program with 'aegis run', not 'aegis build'\n";
            });
            break;
        }
        default:
